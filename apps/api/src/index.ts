...
app.post('/api/export-workflows', async (req, res) => {
  if (supabaseAdmin) {
    try {
      const data = await createExportWorkflow(supabaseAdmin, getTenantId(req), req.body as Record<string, unknown>);
...
function getSuppliedAgentApiKey(req: Request) {
...
function getTenantId(req: Request) {
  ...
  if (typeof requestedTenant === 'string' && requestedTenant.trim()) return requestedTenant.trim();
...
function buildAgentInstallCommand(apiKey: string) {
