...
export type CloudResourceMetric = {
  resourceId: string;
  cpuP95: number;
  cpuP99: number;
  memoryP95?: number;
  ...
  runningHours30d: number;
  nonProdAfterHoursPercent?: number;
};

export type CloudResourceForRightsizing = {
  id: string;
  name: string;
  provider: CloudProviderCode;
  accountName: string;
  service: string;
  resourceType: string;
  sku: string;
...
export type RightsizingRecommendation = {
  ...
  recommendationType:
    | 'Downsize oversized compute'
    | 'Upsize under-provisioned compute'
    | 'Terminate idle resources'
    | 'Stop non-production resources out of hours'
    | 'Resize storage'
    | 'Delete unattached disks'
    | 'Delete stale snapshots'
    | 'Optimise storage tiers'
    | 'Identify commitment savings opportunities';
  reason: string;
  ...
  riskScore: number;
  status: 'New' | 'Reviewing' | 'Approved' | 'Rejected' | 'Implemented' | 'Snoozed';
  owner: string;
  team: string;
};
...
export function generateRightsizingRecommendations(
  resources: CloudResourceForRightsizing[],
  metrics: CloudResourceMetric[]
...
function isCompute(resource: CloudResourceForRightsizing) {
  return resource.resourceType.toLowerCase().includes('compute') || resource.service.toLowerCase().includes('compute');
}

function recommendation(
  resource: CloudResourceForRightsizing,
  input: {
    type: RightsizingRecommendation['recommendationType'];
    sku: string;
    reason: string;
    metrics: string;
    savingRate: number;
...
function metricSummary(metric: CloudResourceMetric) {
