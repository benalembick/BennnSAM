...
import { createRequireAuth, requireRole, requireSuperAdmin, getAuth, DEMO_CREDENTIALS, updateDemoUser } from './auth.js';
import { sendInviteEmail, smtpConfigured } from './email.js';
loadEnvironment();
const app = express();
...
app.post('/api/auth/login', async (req, res) => {
    ...
    await writeAuditLog(supabaseAdmin, {
        tenantId: data.user.app_metadata?.tenant_id ?? defaultTenantId,
...
app.patch('/api/admin/tenant/logo', requireAuth, requireRole('tenant_admin'), async (req, res) => {
    ...
    if (!supabaseAdmin) {
        if (logoUrl)
            demoTenantLogos.set(auth.tenantId, logoUrl);
        else
            demoTenantLogos.delete(auth.tenantId);
        return res.json({ ok: true });
    }
    const { error } = await supabaseAdmin
        .from('tenants')
...
app.post('/api/admin/users/invite', requireAuth, requireRole('tenant_admin'), async (req, res) => {
    ...
    const { data: existingProfile } = await supabaseAdmin
        ...
        .maybeSingle();
    if (existingProfile) {
        const msg = existingProfile.status === 'invited'
    ...
    res.status(201).json(mapUserProfileRow(profile));
});
...
app.patch('/api/admin/users/:id', requireAuth, requireRole('tenant_admin'), async (req, res) => {
    ...
    res.json(mapUserProfileRow(data));
});
...
app.get('/api/superadmin/tenants', requireAuth, requireSuperAdmin, async (_req, res) => {
    ...
    const userCounts = countBy(usersResult.data);
    const deviceCounts = countBy(devicesResult.data);
    const agentCounts = countBy(agentResult.data);
    const enriched = (tenants ?? []).map((t) => ({
        ...t,
        logoUrl: t.logo_url ?? null,
        disabledAt: t.disabled_at ?? null,
        enrollmentKey: t.enrollment_key ?? null,
...
app.patch('/api/superadmin/tenants/:id', requireAuth, requireSuperAdmin, async (req, res) => {
    ...
    if (parsed.data.logoUrl !== undefined) {
        ...
        if (!supabaseAdmin) {
            if (logoUrl)
                demoTenantLogos.set(tenantId, logoUrl);
            else
                demoTenantLogos.delete(tenantId);
            return res.json({ ok: true });
        }
        const { error } = await supabaseAdmin
            .from('tenants')
...
app.post('/api/export-workflows', requireAuth, requireRole('tenant_admin', 'asset_manager'), async (req, res) => {
    ...
    if (supabaseAdmin) {
        try {
            const data = await createExportWorkflow(supabaseAdmin, auth.tenantId, req.body);
...
app.get('/api/cloudability', requireAuth, requireRole('tenant_admin', 'finance_user'), (_req, res) => res.json(cloudabilityPayload()));
...
function hashAgentApiKey(k) { return createHash('sha256').update(k, 'utf8').digest('hex'); }
function getKeyPrefix(k) { return `${k.slice(0, 18)}...`; }
function buildAgentInstallCommand(k) {
...
async function writeAuditLog(client, opts) {
    try {
...
function resolveInviteRedirectUrl() {
    if (process.env.INVITE_REDIRECT_URL)
        return process.env.INVITE_REDIRECT_URL;
...
async function resolveTenantName(tenantId) {
    if (!supabaseAdmin)
        return getDemoTenantName(tenantId);
...
function getDemoTenantName(tenantId) {
    const map = {
...
const demoTenantLogos = new Map();
function getDemoTenantLogoUrl(tenantId) {
