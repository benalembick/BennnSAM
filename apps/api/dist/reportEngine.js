import { applications, complianceResults, licences, saasDetections, savingsRecommendations, usageEvents } from './demoData.js';
const currency = new Intl.NumberFormat('en-AU', {
    style: 'currency',
    currency: 'AUD',
    maximumFractionDigits: 0
});
export function runAssistantQuery(prompt) {
    ...
    if (normalized.includes('unused') || normalized.includes('underused')) {
        ...
        return {
            ...
            summary: `Found ${rows.length} optimisation opportunities with a combined annual value of ${currency.format(rows.reduce((sum, row) => sum + Number(row.annualSavings), 0))}.`,
            chartType: 'bar',
            rows
        };
    }
